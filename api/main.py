...
async def list_verifier_definitions(system: Optional[str] = None, type: Optional[str] = None, environment: Optional[str] = None):
    ...
    db_verifiers = {v["id"]: v for v in _verifier_db_store.list_all() if v.get("id")}
...
async def get_verifier_definition(verifier_id: str):
    ...
    v = _verifier_store.get(verifier_id) or _verifier_db_store.get(verifier_id)
    if not v:
...
async def edit_verifier_definition(verifier_id: str, request: VerifierDefinition):
    ...
    existing = _verifier_store.get(verifier_id) or _verifier_db_store.get(verifier_id)
    if not existing:
        raise HTTPException(status_code=404, detail=f"Verifier {verifier_id} not found")
    vdata = request.dict()
...
async def disable_verifier(verifier_id: str):
    ...
    _verifier_db_store.upsert(verifier_id, existing)
...
from api.persistence import EnvironmentStore, ScenarioStore, VerifierStore, migrate_json_to_store  # noqa: E402

_env_store = EnvironmentStore()
...
_migrated = migrate_json_to_store(_CUSTOM_ENV_STORE_PATH, _env_store)
