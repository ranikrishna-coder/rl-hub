    ...
    try {
        ...
        if (!allEnvironments.some(e => e.name === 'ClinKriya Clinic')) {
            allEnvironments.push({
                ...
                description: 'Simulates a primary care clinic (PCP) where an agent handles real-world clinical tasks by interacting with a live FHIR EHR server.',
                category: 'cross_workflow',
                system: 'FHIR / EHR',
                sdk: 'docker',
                source: 'huggingface',
...
function createEnvCard(env) {
    ...
    for (var ci = 0; ci < CARD_CHIP_FIELDS.length; ci++) {
        ...
        if (field.format === 'badge') {
            ...
            // For workflow field, split on commas to render multiple chips
                    ...
                    if (body) {
                        ...
                        if (contentDiv) {
                            ...
                            filtered.forEach(function (s, idx) {
                                var actBtns = '<div class="card-action-btns" onclick="event.stopPropagation()">' +
        ...
        .then(function (data) {
            ...
            if (added > 0) {
                ...
                if (section) {
                    ...
                    var countSpan = section.querySelector('.detail-collapsible-header h2 span');
                    if (countSpan) countSpan.textContent = '(' + filtered.length + ')';
                    // Re-render verifier cards
                    var body = document.getElementById('section-verifiers-body');
                    if (body) {
                        var contentDiv = body.querySelector('.detail-collapsible-content');
                        if (contentDiv) {
                            var listHtml = '';
                            filtered.forEach(function (v, idx) {
                                ...
                                var vActBtns = '<div class="card-action-btns" onclick="event.stopPropagation()">' +
                            ...
                            var verifiersList = contentDiv.querySelector('.verifiers-list');
                            if (verifiersList) {
                                verifiersList.innerHTML = listHtml;
                            }
                        }
                    }
                }
...
function _buildSimulationsContent(envName, env, details) {
    if (details.source === 'huggingface' || env.source === 'huggingface') {
        ...
        return (tagsHtml ? '<div class="hf-tags-row">' + tagsHtml + '</div>' : '') +
            ...
            (hfUrl && hfOwner && hfRepo ? (function () {
                var basePath = (details.frontMatter || {}).base_path || (openenv || {}).base_path || '';
                var iframeSrc = 'https://' + hfOwner + '-' + hfRepo + '.hf.space' + basePath;
                return '<div class="hf-iframe-wrap"><iframe id="hf-app-iframe" class="hf-iframe" src="' + iframeSrc + '" allow="accelerometer; camera; encrypted-media; geolocation; gyroscope; microphone" sandbox="allow-scripts allow-same-origin allow-forms allow-popups" loading="lazy"></iframe></div>' +
...
function editEnvironment(envName) {
    ...
    var workflowParts = currentWorkflow ? currentWorkflow.split(',').map(function (w) { return w.trim(); }).filter(Boolean) : [];
